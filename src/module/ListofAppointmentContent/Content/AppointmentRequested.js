...
function AppointmentRequested({ listRequested }) {
  const navigate = useNavigate();
  const [currentPage, setCurrentPage] = useState(1);
  const [itemsPerPage, setItemsPerPage] = useState(10);
  const indexOfLastItem = currentPage * itemsPerPage;
  const indexOfFirstItem = indexOfLastItem - itemsPerPage;
  const [listData, setListData] = useState(listRequested);
  const [listOrigin, setListOrigin] = useState(listRequested);
  useEffect(() => {
    setListOrigin(listRequested);
    setListData(listRequested);
  }, [listRequested]);
